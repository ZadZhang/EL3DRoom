...
msj3D.prototype.initThree = function () {
  ...
  _this.renderer.domElement.addEventListener('mousemove', _this.onDocumentMouseStop, false);
...
msj3D.prototype.CreateWall = function (_this, _obj) {
  ...
  var _commonLength = _obj.length || 100;//墙体长度
  ...
  $.each(_obj.wallData, function (index, wallobj) {
    var wallLength = wallobj.length || _commonLength;
...
msj3D.prototype.createCube = function (_this, _obj) {
  ...
  //六面纹理 THREE.FaceColors —— 着色器使用材质的 color 属性着色每一个面
  var skin_up_obj = {
    vertexColors: THREE.FaceColors
  }
...
msj3D.prototype.createEmptyCabinet = function (_this, _obj) {
  ...
  if (_obj.userData != 'undefined' && _obj.userData != null) {
    Cabinet.userData = _obj.userData;
  }
  ...
  if (_obj.doors != null && typeof (_obj.doors) != 'undefined') {
    ...
    if (doors.skins.length == 1) {//单门
      ...
      if (_this.commonFunc.hasObj(doors.children)) {
        var _newobj = null;
        _newobj = _this.createTitleBoard(_this, _obj);
        // singledoorcube = _this.mergeModel(_this, doors.children.op, singledoorcube, _newobj);
      }
...
msj3D.prototype.createCanva2D = function (parameters) {
  if (parameters === undefined) parameters = {};
  var canvasWidth = parameters.width || 60;
  var canvasHeight = parameters.height || 20;
  var fontface = parameters.hasOwnProperty("fontface") ? parameters["fontface"] : "Arial";
  var fontsize = parameters.hasOwnProperty("fontsize") ? parameters["fontsize"] : 16;
  var textColor = parameters.hasOwnProperty("textColor") ? parameters["textColor"] : { r: 160, g: 0, b: 0, a: 1.0 };
  var message = parameters.hasOwnProperty("message") ? parameters["message"] : "备用";
  ...
  msj3D.prototype.createSkinOptionOnj = function (_this, flength, fwidth, _obj, _cube, _cubefacenub) {
    if (_this.commonFunc.hasObj(_obj)) {
      ...
      } else if (_obj.message) {
        return {
          map: _this.createCanva2D(_obj)
        }
      } else {
  ...
  msj3D.prototype.createTitleBoard = function (_this, _obj) {
    if (_this == null) {
      _this = this;
    }
    var _commonSkin = 0xFFFFFF;
    var cubeobj = {
      testFlag: true,
      show: true,
      uuid: _obj.uuid,
      name: _obj.doors.children.name,
      objType: 'cube',
      length: _obj.size.thick + 1,
      width: _obj.size.width,
      height: _obj.size.height - 180,
      rotation: 0,
      x: 0,
      y: 90,
      z: 0,
...
//添加模态框
msj3D.prototype.addModal = function (_obj) {
  var _this = msj3DObj;
  if (_obj != null) {
    $("#" + _this.fId).after('<div id="toolbar" class="toolbar" ></div>');
    $.each(_btnobjs, function (index, _obj) {
...
var _curObjName = '';
var timeFlag;
msj3D.prototype.onDocumentMouseStop = function (event) {
  event.preventDefault();
  var _this = msj3DObj;
  _this.raycaster.setFromCamera(_this.mouseClick, _this.camera);
  var intersects = _this.raycaster.intersectObjects(_this.objects);
  if (intersects.length > 0) {
    ...
    if (_obj.name.indexOf("equipName") >= 0 && _curObjName == '') {
      console.log(_obj);
      _curObjName = _obj.name;
      // TODO: 修改文字显示方式加底纹及改变字体和颜色
      $("#hint").css('left', event.clientX + 8 + 'px');
      $("#hint").css('top', event.clientY + 2 + 'px');
      $("#hint").text(_curObjName.split('_')[0]);
      $("#hint").show();
      _obj = '';
    } else {
      _curObjName = '';
